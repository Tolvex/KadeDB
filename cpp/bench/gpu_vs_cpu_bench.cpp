...
static TableSchema makeRelSchema(int num_columns = 3) {
  ...
  for (int i = 1; i < num_columns; ++i) {
    cols.push_back(
        Column{"x" + std::to_string(i), ColumnType::Integer, false, false, {}});
  }
...
static TimeSeriesSchema makeTsSchema(int num_value_columns = 1) {
  ...
  for (int i = 0; i < num_value_columns; ++i) {
    s.addValueColumn(Column{
        "value" + std::to_string(i), ColumnType::Float, false, false, {}});
  }
  return s;
}
...
static BenchmarkResult benchmarkCpuScanFilter(InMemoryRelationalStorage &rel,
                                              ...
                                              int64_t rhs, Predicate::Op op,
                                              int64_t num_rows) {
  BenchmarkResult result;

  result.cpu_time_ms = time_ms([&]() {
    // Build predicate inside lambda to ensure it's fresh for each call
    Predicate pred;
    ...
    auto res = rel.select(table, /*columns=*/{},
...
static BenchmarkResult benchmarkCpuTimeRange(InMemoryTimeSeriesStorage &ts,
  ...
  // Note: InMemoryTimeSeriesStorage doesn't expose getSeriesInfo, so we
  // estimate
  result.rows_processed = num_rows; // Use estimated row count
...
static BenchmarkResult benchmarkCpuTimeAggregation(
    InMemoryTimeSeriesStorage &ts, const std::string &series,
    const std::string &value_column, TimeAggregation agg, int64_t start,
    int64_t end, int64_t bucket_width, int64_t num_rows) {
  BenchmarkResult result;

  result.cpu_time_ms = time_ms([&]() {
    auto res = ts.aggregate(series, value_column, agg, start, end, bucket_width,
                            TimeGranularity::Seconds, std::nullopt);
    if (!res.hasValue()) {
...
benchmarkGpuScanFilter(const std::vector<int64_t> &column_data, int64_t rhs,
                       GpuScanSpec::Op op) {
  BenchmarkResult result;
  ...
  result.gpu_time_ms = time_ms([&]() {
    result.transfer_to_gpu_ms =
        0.0; // Placeholder - actual transfer timing needed
    auto gpu_result = gpuScanFilterInt64(spec);
    result.output_rows = gpu_result.size();
    result.transfer_from_gpu_ms = 0.0; // Placeholder
  });

  result.gpu_compute_ms = result.gpu_time_ms - result.transfer_to_gpu_ms -
                          result.transfer_from_gpu_ms;

  return result;
}

static BenchmarkResult
benchmarkGpuTimeRange(const std::vector<int64_t> &timestamps, int64_t start,
                      int64_t end) {
  BenchmarkResult result;

  // Use two-phase filter: first >= start, then < end
  result.rows_processed = timestamps.size();

  result.gpu_time_ms = time_ms([&]() {
    result.transfer_to_gpu_ms = 0.0; // Placeholder

    // Phase 1: Filter timestamps >= start
    GpuScanSpec specStart;
    specStart.column = timestamps.data();
    specStart.count = timestamps.size();
    specStart.rhs = start;
    specStart.op = GpuScanSpec::Op::Ge;
    auto indicesGe = gpuScanFilterInt64(specStart);

    // Phase 2: From those indices, keep only < end
    // For simplicity, we scan the full range again with < end
    // In a real GPU implementation this would be done more efficiently
    GpuScanSpec specEnd;
    specEnd.column = timestamps.data();
    specEnd.count = timestamps.size();
    specEnd.rhs = end;
    specEnd.op = GpuScanSpec::Op::Lt;
    auto indicesLt = gpuScanFilterInt64(specEnd);

    // The result size is the intersection count (approximate for benchmarking)
    // For proper benchmarking we should track actual intersection
    result.output_rows = indicesLt.size();
    result.transfer_from_gpu_ms = 0.0; // Placeholder
  });
...
static BenchmarkResult
benchmarkGpuTimeAggregation(const std::vector<int64_t> &timestamps,
                            const std::vector<double> &values, int64_t start,
                            int64_t end, int64_t bucket_width) {
  BenchmarkResult result;
...
// Utility functions
static void printBenchmarkResult(const std::string &test_name,
                                 BenchmarkResult &result) {
  ...
  if (result.cpu_time_ms > 0 && result.gpu_time_ms > 0) {
  ...
  if (result.gpu_time_ms > 0) {
    std::cout << "  GPU breakdown - Transfer to: " << result.transfer_to_gpu_ms
              ...
              << " ms, Transfer from: " << result.transfer_from_gpu_ms
              << " ms\n";
  }
  ...
  if (result.cpu_time_ms > 0) {
    double cpu_throughput =
        result.rows_processed / (result.cpu_time_ms / 1000.0);
    std::cout << "  CPU throughput: " << static_cast<int64_t>(cpu_throughput)
              << " rows/sec\n";
  }

  if (result.gpu_time_ms > 0) {
    double gpu_throughput =
        result.rows_processed / (result.gpu_time_ms / 1000.0);
    std::cout << "  GPU throughput: " << static_cast<int64_t>(gpu_throughput)
              << " rows/sec\n";
  }
...
int main(int argc, char **argv) {
  ...
  int num_columns = (argc > 2) ? static_cast<int>(parseInt64(argv[2], 3)) : 3;

  // Validate: need at least 2 columns (id + x1) for the benchmark to work
  if (num_columns < 2) {
    std::cerr << "Warning: num_columns must be >= 2 (need at least 'id' and "
                 "'x1'). Clamping to 2.\n";
    num_columns = 2;
  }

  std::cout << "KadeDB GPU vs CPU Benchmark Suite\n";
  std::cout << "Dataset: " << num_rows << " rows, " << num_columns
            << " columns\n\n";

  // Check GPU availability
  auto gpu_status = gpuStatus();
  std::cout << "GPU Status: "
            << (gpu_status.available ? "Available" : "Not Available") << " - "
            << gpu_status.message << "\n\n";
  ...
  // === Test 1: Scan-only benchmark ===
  {
    ...
    // Pre-generate data to ensure CPU and GPU use the same dataset
    std::cout << "Generating " << num_rows << " rows...\n";
    std::vector<std::vector<int64_t>> pre_generated_data(num_rows);
    for (int64_t i = 0; i < num_rows; ++i) {
      for (int j = 0; j < num_columns - 1; ++j) {
        pre_generated_data[i].push_back(id_dist(rng));
      }
    }

    // Extract x1 column for GPU benchmarking (same data as CPU)
    std::vector<int64_t> column_data(num_rows);
    for (int64_t i = 0; i < num_rows; ++i) {
      column_data[i] = pre_generated_data[i][0]; // x1 is at index 0 of values
    }

    double insert_time = time_ms([&]() {
      for (int64_t i = 0; i < num_rows; ++i) {
        (void)rel.insertRow("t", makeRelRow(i, pre_generated_data[i]));
      }
    });
    ...
    BenchmarkResult cpu_result =
        benchmarkCpuScanFilter(rel, "t", "x1", 0, Predicate::Op::Ge, num_rows);
    printBenchmarkResult("CPU Full Scan", cpu_result);

    // GPU equivalent (currently CPU fallback) - uses same column_data
    BenchmarkResult gpu_result =
        benchmarkGpuScanFilter(column_data, 0, GpuScanSpec::Op::Ge);
    ...
    combined.speedup = (gpu_result.gpu_time_ms > 0)
                           ? (cpu_result.cpu_time_ms / gpu_result.gpu_time_ms)
                           : 0.0;
  ...
  // === Test 2: Filter-selective benchmark ===
  {
    ...
    for (auto [threshold, expected_selectivity] : selectivities) {
      std::cout << "--- Selectivity ~" << expected_selectivity << "% (x < "
                << threshold << ") ---\n";
      ...
      for (int64_t i = 0; i < num_rows; ++i) {
        ...
        for (int j = 0; j < num_columns - 1; ++j) {
          values.push_back(
              i % 1000000); // Predictable pattern for consistent selectivity
        }
        (void)rel.insertRow("t", makeRelRow(i, values));
      }

      BenchmarkResult cpu_result = benchmarkCpuScanFilter(
          rel, "t", "x1", threshold, Predicate::Op::Lt, num_rows);
      printBenchmarkResult(
          "CPU Filter " +
              std::to_string(static_cast<int>(expected_selectivity)) + "%",
          cpu_result);
      ...
      BenchmarkResult gpu_result =
          benchmarkGpuScanFilter(column_data, threshold, GpuScanSpec::Op::Lt);
      printBenchmarkResult(
          "GPU Filter " +
              std::to_string(static_cast<int>(expected_selectivity)) + "%",
          gpu_result);
  ...
  // === Test 3: Time range query benchmark ===
  {
    ...
    for (auto [window_size, window_name] : windows) {
      ...
      BenchmarkResult cpu_result =
          benchmarkCpuTimeRange(ts, "s", start, end, num_rows);
      ...
      BenchmarkResult gpu_result =
          benchmarkGpuTimeRange(timestamps, start, end);
  ...
  // === Test 4: Time bucket aggregation benchmark ===
  {
    ...
    for (auto [bucket_width, bucket_name] : bucket_configs) {
      ...
      BenchmarkResult cpu_result =
          benchmarkCpuTimeAggregation(ts, "s", "value0", TimeAggregation::Sum,
                                      start, end, bucket_width, num_rows);
      ...
      BenchmarkResult gpu_result = benchmarkGpuTimeAggregation(
          timestamps, values, start, end, bucket_width);
  ...
  std::cout << "GPU available: " << (gpu_status.available ? "Yes" : "No")
            << "\n";
  std::cout << "Go/No-Go status: "
            << (all_go_no_go_passed ? "✅ PASSED" : "❌ FAILED") << "\n";

  if (!all_go_no_go_passed) {
    std::cout << "\n⚠️  Some benchmarks did not meet the 2x speedup "
                 "threshold on >=10M rows.\n";
    std::cout << "This may indicate that GPU optimization needs more work or "
                 "that the\n";
    std::cout << "current implementation is already CPU-optimized for these "
                 "workloads.\n";
  }
